...
### 2.3 `POST /auth/google` · `POST /auth/kakao` — 소셜 로그인
...
|---|---|
| `GET /health`, `GET /` | ✅ 동작 중 |
| 인증 10종 (회원가입·로그인·소셜·refresh·로그아웃) | ✅ **동작 중** (2026-08-22) |
...
**엔드포인트 19종이 실제로 붙었습니다.** 서버를 띄우면 바로 호출할 수 있고,
...
> 의존하지 않게** 해주세요.
> 소셜 로그인은 서버 쪽 검증이 끝났고, 앱에 카카오/구글 SDK를 붙여 받은 토큰만 넘겨주면 됩니다.
