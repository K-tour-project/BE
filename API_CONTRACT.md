...
- 후보가 2개 이상이면 **AI가 사용자에게 되물어야 합니다** ("1981년 만추 말씀이신가요?").
...
| 이름 | 필수 | 설명 |
|---|---|---|
| `content_id` | | **작품으로 좁히기.** 유저플로우 4번 "지도에서 영화 하나 선택"에 해당 — 강릉 지도에서 「관상」을 탭하면 `?content_id=77` 로 재요청하면 강릉 안의 관상 촬영지만 남는다 |
| `limit` / `offset` | | 페이지네이션 |
...
| 작품·촬영지 2종 | ⬜ 5단계 |
| 지역·지도 2종 | ⬜ 5단계 |
| `GET /places/{id}` (TourAPI) | ⬜ 4단계 — 인증키 발급 대기 |
| 코스 5종 | ⬜ 6단계 |

**아직 서버가 없어도 개발할 수 있습니다** — 위 JSON 예시를 그대로 목(mock)으로 쓰세요.
전부 실제 DB에서 뽑은 값이라 형태가 바뀌지 않습니다.
