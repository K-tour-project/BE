...
| # | 화면 · 동작 | 부를 API | 담당 |
|---|---|---|---|
| 0 | **홈 → 인기 작품·관광지 10개씩** | `GET /home` | 윤영 |
| 1 | 챗봇 인터페이스로 시작 | (AI 내부) | 조시현 |
| 2 | 홈에서 **작품으로 찾기** 또는 **지역으로 찾기** 선택 | 아래 3·4로 분기 | 윤영/조시현 |
| 3 | **작품 검색 → 작품 목록 출력** | `GET /contents/search?q=` | 윤영 |
| 4 | **지역 선택/검색 → 관광지와 촬영지로 분류된 관광지** | `GET /regions/sidos` → `GET /regions/{sido_id}/children`, 또는 `GET /regions/resolve?name=` → `GET /regions/{region_id}/tourism-places` | 윤영 |
...
`GET /home`은 로그인 없이 호출합니다. `popular_products`는 작품 별점순 10개이며,
`popular_tourism_places`는 무료·유료 구분 없이 앱 찜 수 순으로 최대 10개입니다.
찜 데이터가 부족하면 대표 이미지가 있는 TourAPI 최근 수정 관광지로 10개까지 채웁니다.
각 항목의 `detail_path`로 기존 작품·관광지 상세 API를 호출합니다.

응답 필드와 예시는 [`docs/HOME_API.md`](docs/HOME_API.md)를 기준으로 합니다.
...
## 마이페이지·찜·작품 저장·프로필 설정

[마이페이지 API 계약](docs/MYPAGE_API.md)에 화면 진입 응답, 탭별 목록,
촬영지·일반 관광지 찜과 취소, 작품 저장과 취소, 프로필 URL 수정,
기존 상세 화면 연결 방법을 정리했습니다.
