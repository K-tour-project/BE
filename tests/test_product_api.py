...
class ProductApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_filming_locations_include_regions_and_place_link(self):
        ...
        items, total = await _filming_locations(db, 3)
    ...
    async def test_filming_locations_do_not_require_tour_api_match(self):
        db = AsyncMock()
        db.scalar.return_value = 1
        db.execute.return_value = Mock(all=Mock(return_value=[SimpleNamespace(
            place_id=8, tour_content_id=None, name="Place", region_name=None, parent_name=None
        )]))
        items, total = await _filming_locations(db, 3)
        self.assertEqual(total, 1)
        self.assertIsNone(items[0].tour_content_id)
        self.assertEqual(items[0].detail_path, "/places/8")

    async def test_place_uses_database_category_and_internal_id(self):
        db = AsyncMock()
        db.execute.return_value = Mock(all=Mock(return_value=[SimpleNamespace(
            place_id=10, product_id=42, title="Movie", category="MOVIE", poster_url="poster"
        )]))
        ...
        self.assertIn("product_places.place_id", str(db.execute.call_args.args[0]))

    async def test_place_products_are_isolated_by_place_id(self):
        db = AsyncMock()
        db.execute.return_value = Mock(all=Mock(return_value=[SimpleNamespace(
            place_id=10, product_id=42, title="Movie", category="MOVIE", poster_url=None
        )]))
        result = await _contents_by_place(db, [10, 11])
        self.assertEqual([item.product_id for item in result[10]], [42])
