...
> 과거 구현 과정과 설계 판단을 기록한 보관 문서다. 현재 동작과 API는 코드 및
> [`API_CONTRACT.md`](./API_CONTRACT.md)를 기준으로 확인한다.
...
### ↳ 후속 (2026-08-22) — 카카오 로그인을 윤영에게 이관

위 기록 시점엔 카카오도 구현돼 있었다(엔드포인트 10종·38케이스). 이후 팀 회의에서
**카카오는 윤영이 맡기로** 분담이 바뀌어 서버 코드를 덜어냈다.

**왜** — 윤영이 카카오맵 API 때문에 카카오 개발자센터 앱을 이미 만들어 뒀고, 그 앱 하나에
로그인이 같이 붙는다. 콘솔·앱 SDK를 쥔 쪽이 서버 엔드포인트까지 맡는 편이 왕복이 적다.
구글은 콘솔이 별개(Google Cloud)라 김은서가 계속 담당한다.

**무엇을 덜어냈나** — `social.verify_kakao()` 본문과 `router`의 `POST /auth/kakao` 뿐이다.
지운 코드는 커밋 `160ea64`에 남아 있어 필요하면 되살릴 수 있다.

**무엇을 남겼나** — 공용이라 손대지 않았다. 카카오를 붙일 때 **새로 만들 게 거의 없다.**
`service.social_login()`(회원 조회·자동가입·토큰 발급, 구글과 공용) · `SocialProfile` ·
`schema.KakaoLoginRequest`(계약서 §2.3에 이미 프론트와 약속된 형태) · `AuthProvider.kakao`
enum · `settings.KAKAO_APP_ID`.

→ 구현할 것은 **`verify_kakao()` 함수 하나 + 라우터 4줄**이다. 호출할 URL·응답 예시·
주의점(앱 소속 `app_id` 대조가 왜 핵심인지, 이메일이 null일 수 있는 이유, 401 vs 502)은
`social.py`의 해당 docstring에 전부 적어뒀다.

**⚠️ 남겨둔 함정 하나** — 서버가 검증에 쓰는 구글 값은 **웹 애플리케이션 클라이언트 ID**다
(Android용이 아니다). 안드로이드 구글 로그인은 `setServerClientId(웹클라이언트ID)`로 넘기고
그 값이 `id_token`의 `aud`에 찍히기 때문이다. Android 클라이언트 ID를 `.env`에 넣으면
**검증이 항상 실패**한다.

**결과** — 엔드포인트 9종, `check_auth.py` **37케이스 전부 통과**(카카오 위조토큰 케이스만 주석 처리).
