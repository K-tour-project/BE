...
| 폴더 | 파일 | 역할 |
|---|---|---|
| **auth/** | `router.py` | 엔드포인트 9종 (+ 카카오 자리). 얇게 — 판단은 service로 넘긴다 |
...
| ~~`POST /auth/kakao`~~ | `{access_token}` | 토큰쌍 | ⬜ **윤영 담당 — 미구현.** 계약 형태는 확정됨 |
...
| **3** | **인증 9종** (카카오는 윤영) | ✅ **08-22** | `features/auth/`, 리비전 `9a1c7d2e5b40` |
| 6 | 코스 추천 엔진 | ⬜ | |
| 8 | 테스트·하드닝 | ⬜ | pytest 0개 |
| 9 | 배포·제출 | ⬜ | |
...
`check_auth.py` 37케이스 중 **성공 9 / 거부 28**인 게 의도다.
...
*갱신 이력 — 2026-06 최초(1단계) / 2026-08-15 구조·DB 반영 / **2026-08-22 전면 재작성**(기능별 폴더 구조·인증·10테이블 반영). 이후 카카오 로그인은 윤영 담당으로 이관돼 인증 9종.*
